...
async function refreshAccessToken() {
  ...
  if (!refreshToken) {
    // ✅ CAMBIO: sin refresh token → limpiar todo para forzar nuevo login de Spotify
  ...
  // ✅ CAMBIO: refresh falló (token revocado/expirado) → limpiar todo
  localStorage.removeItem('spotify_token');
  localStorage.removeItem('spotify_token_expiry');
...
export async function getToken() {
  ...
  // ✅ CAMBIO: renueva si expiró o expira en menos de 5 minutos (antes 2 min)
  if (Date.now() > expiry - 300_000) {
    const refreshed = await refreshAccessToken();
    if (refreshed) return refreshed;
    // ✅ CAMBIO: si no se pudo renovar, lanza error en vez de devolver token vencido
